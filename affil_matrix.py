...
affil.to_csv('affiliation_matrix_-1.csv') #initial csv
...
while len(usernames_index_list) != 0:
    user = usernames_index_list.pop(0) #pop out the user
    try:
        ...
        affil.to_csv(f'affiliation_matrix_{str(success)}.csv') #new file
        os.remove(f'affiliation_matrix_{str(success-1)}.csv') #remove old file
        success+=1
    except:
